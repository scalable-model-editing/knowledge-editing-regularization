...
def execute_encore_with_gradient_descent(
    ...
    for r_id, request in enumerate(requests):
        ...
        if not data_loaded:
            ...
            if cache_fname is not None:
                ...
                print(f"Cached k/v pair at {cache_fname}")
    zs = torch.stack(z_list, dim=1) #[1600, 1]
    ...
    for i, layer in enumerate(hparams.layers):
        ...
        # Get current model activations
        layer_ks = compute_ks(model, tok, requests, hparams, layer, context_templates).T
        input_keys = get_module_input_output_at_words(
            model,
            tok,
            layer,
            context_templates=[request["prompt"] for request in requests],
        ...
        residual = get_module_input_output_at_words(
            model,
            tok,
            layer,
            context_templates=[request["prompt"] for request in requests],
            words=[request["subject"] for request in requests],
            module_template="transformer.h.{}.ln_2",
            fact_token_strategy=hparams.fact_token,
        )[0].T

        #mlp_output_cal = layer_weight.T @ input_keys + layer_bias 
        #layer_output_cal = mlp_output_cal + residual
        ...
        # Adjust update matrix shape
        weight_name = f"{hparams.rewrite_module_tmp.format(layer)}.weight"
        W_original = nethook.get_parameter(model, f"{hparams.rewrite_module_tmp.format(layer)}.weight").detach().clone()
        bias_original= nethook.get_parameter(model, f"{hparams.rewrite_module_tmp.format(layer)}.bias").detach().clone().reshape(-1, 1)

        W_edited = gradient_descent_update(
            W_edited=W_original.clone(),
            W_original=W_original,
            bias_original=bias_original,
            residual=residual,
            key_preserved=preserved_keys,  # Obtained from layer_stats_dynamic
            key_edited=input_keys,  # Computed or passed from previous code
            value_edited=zs,  # The target values for editing
            lambda_value=hparams.gd_lambda, #grad descent lambda
            learning_rate=hparams.grad_desc_lr,
            num_iterations=hparams.grad_desc_steps,
            tolerance = 1e-6, 
            hparams = hparams
        )

        #deltas[weight_name] = (W_edited - W_original).detach()

        deltas[weight_name] = (
                (W_edited - W_original).detach(),
                None,
                None, 
                None, 
                None,
...
def gradient_descent_update(
    W_edited: torch.Tensor,
    W_original: torch.Tensor,
    bias_original: torch.Tensor,
    residual: torch.Tensor,
    key_preserved: torch.Tensor,
    key_edited: torch.Tensor,
    value_edited: torch.Tensor,
    lambda_value: float,
    ...
    """
    Perform gradient descent to optimize W_edited.

    :param W_edited: The initial weight matrix we are optimizing.
    :param W_original: The original weight matrix.
    :param key_preserved: The keys that need to be preserved.
    :param key_edited: The keys that need to be edited.
    :param value_edited: The target values for the edited keys.
    :param learning_rate: The learning rate for gradient descent.
    :param num_iterations: Number of iterations for gradient descent.
    :param tolerance: Early stopping tolerance for loss.
    :param retain_graph: Whether to retain the computational graph for multiple backward passes.
    :return: The optimized weight matrix.
    """

    W_edited = W_edited.clone().detach().requires_grad_(True)
    optimizer = torch.optim.Adam([W_edited], lr=learning_rate)
    loss_meter = AverageMeter()

    #calculate original spectral norm
    _, S, _ = torch.linalg.svd(W_original, full_matrices=False)
    spectral_norm_original = torch.max(S)
    condition_num_original = torch.max(S)/ torch.min(S)

    for iteration in range(num_iterations):
        loss_meter.reset()
        optimizer.zero_grad()
        key_preserved = key_preserved.detach()
        key_edited = key_edited.detach()
        value_edited = value_edited.detach()

        # Compute the loss function in a new forward pass
        loss, pres_loss, edit_loss = compute_loss(W_edited, W_original, key_preserved, key_edited, value_edited, bias_original, residual, lambda_value)
        
        #calculate spectral norm
        _, S, _ = torch.linalg.svd(W_edited, full_matrices=False)
        spectral_norm_edit = torch.max(S)
        spectral_norm_loss = torch.abs(spectral_norm_original - spectral_norm_edit)
        loss = loss + hparams.spectral_norm * spectral_norm_loss

        #condition number constraint
        condition_num_edit = torch.max(S)/ torch.min(S)
        condition_num_loss = torch.abs(condition_num_original - condition_num_edit)
        loss = loss + hparams.condition_num * condition_num_loss
        ...
        print(f"Iteration {iteration + 1}/{num_iterations}, Loss: {round(loss.item(),3)}, Pres Loss: {round(pres_loss.item(),3)}, Edit Loss: {round(edit_loss.item(),3)}, Spectral: {spectral_norm_loss.item()}, Condition: {condition_num_loss.item()}")

        # Early stopping if the loss drops below the specified tolerance
        if loss.item() < tolerance:
            print(f"Early stopping at iteration {iteration + 1} due to loss < tolerance ({tolerance})")
            break
        
    # Return the optimized weights after gradient descent
    return W_edited.detach()


def compute_loss(W_edited, W_original, key_preserved, key_edited, value_edited, bias_original, residual, lambda_value):
    """
    Compute the loss function for gradient descent.
    :param W_edited: The weight matrix we are optimizing.
    :param W_original: The original weight matrix.
    :param key_preserved: The keys that need to be preserved.
    :param key_edited: The keys that need to be edited.
    :param value_edited: The target values for the edited keys.
    ...
    key_preserved = key_preserved.to("cuda")
    if W_original.shape[0] != key_preserved.shape[1]:
        W_original = W_original.T
        W_edited = W_edited.T

    preserved_output_edited = key_preserved.double() @ W_edited.double() 
    preserved_output_original = key_preserved.double() @ W_original.double()

    preservation_loss = torch.norm(preserved_output_edited - preserved_output_original, p = 2)**2 / key_preserved.shape[0]

    ###### COMPUTE EDITING LOSS
    if key_edited.shape[1] != W_edited.shape[0]:
        W_edited = W_edited.T
    
    edited_output_edited = W_edited.double() @ key_edited.double() + bias_original + residual
    editing_loss = torch.norm(edited_output_edited - value_edited, p = 2)**2 / key_edited.shape[1]

    # Total loss is the sum of preservation and editing losses
    total_loss = lambda_value * preservation_loss + editing_loss
    
    return total_loss, preservation_loss, editing_loss
