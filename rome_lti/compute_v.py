...
def compute_v(
    ...
    if hasattr(model.config, 'n_embd'):
        ...
        delta = torch.zeros((model.config.hidden_size,),
                            requires_grad=True, device="cuda")
    target_init, kl_distr_init, midlayer_vec = None, None, None

    # Inserts new "delta" variable at the appropriate part of the computation
    def edit_output_fn(cur_out, cur_layer):
        nonlocal target_init, midlayer_vec
    ...
    for it in range(hparams.v_num_grad_steps):
        ...
        #     hooks = {name: stack.enter_context(hook) for name, hook in named_hooks.items()}
        ...
        nll_loss = hparams.nll_factor * nll_loss_each.mean()
